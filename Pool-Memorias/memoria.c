...
int resolver_insert(request_t req, int modif)
{
	...
	char *valor = malloc(strlen(req.args[2])+1);
	strcpy(valor,req.args[2]);
...
char* select_memoria(char *nombre_tabla, uint16_t key)
{
	...
	if(funcionSelect(nombre_tabla, key, &pagina, &aux)!=-1){
...
void ejecutarHiloConsola() {
	...
	hiloCrear(&hiloConsolaMemoria, (void*)hilo_consola, NULL);
